...
async def validate_origin(raw: str) -> tuple[str, str]:
    try:
        ...
        raw_ips = {ipaddress.ip_address(r[4][0]) for r in records}
        if any(ip.is_private or ip.is_loopback or ip.is_multicast for ip in raw_ips):
            raise ValueError("Private, loopback and reserved addresses are not allowed. Use the hosted local mode or a public tunnel.")
        global_ips = {ip for ip in raw_ips if ip.is_global and not ip.is_reserved}
        if not global_ips:
            raise ValueError("Private, loopback and reserved addresses are not allowed. Use the hosted local mode or a public tunnel.")
        ip = sorted(global_ips, key=lambda i: (i.version, str(i)))[0]
