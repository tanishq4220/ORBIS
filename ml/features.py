...
def extract_tle_features(
    ...
    # SGP4 stores mean motion in rad/min and ndot in rad/min^2.
    # Training features use CelesTrak / TLE units:
    #   MEAN_MOTION      -> revolutions / day
    #   MEAN_MOTION_DOT  -> revolutions / day^2  (TLE n-dot field)
    #
    # Convert with the inverse of sgp4's twoline2rv scaling:
    #   sat.ndot = tle_ndot / (xpdotp * 1440)
    #   xpdotp   = 1440 / (2 * pi)
    ...
    return {
        ...
        "mean_motion_rev_day":
            _safe(sat.no_kozai) * xpdotp,

        "bstar":
            _safe(sat.bstar),

        "mean_motion_derivative":
            _safe(sat.ndot) * xpdotp * 1440.0,
    }
