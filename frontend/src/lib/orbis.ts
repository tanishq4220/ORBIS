...
export interface ScreeningResult { screening_id: string; status: string; prototype?: boolean; disclaimer?: string; method?: string; method_description?: string; target_id: string; target_name?: string | null; timestamp_utc: string; completed_utc?: string; start_utc?: string; time_step_min: number; window_min: number; threshold_km: number; top_n?: number; objects_screened?: number; potential_count?: number; minimum_separation_km: number | null; tca_utc: string | null; results: ScreeningRow[]; full_results?: ScreeningRow[]; error?: string }
