...
import Earth3DSurface from "./globe/Earth3DSurface";
...
function CameraRig({command,state,onLevel,sun}:{command:{kind:string;seq:number};state?:ObjectState;onLevel:(n:number)=>void;sun?:number[]}){
  ...
  // Demand mode needs an initial frame and an ongoing frame request while
  // auto-rotation is active; controls' tiny damped deltas may not emit change.
  useEffect(()=>{if(auto)invalidate();},[auto,invalidate]);
...
export default function EarthGlobe({positions,onSelect,current,trajectory,highlights=[]}:{positions?:PositionsResponse;onSelect:(id:string)=>void;current?:ObjectState;trajectory?:TrajectoryResponse;highlights?:string[]}){
  ...
  return <div data-testid="earth-globe-container" className="globe-stage"><GlobeBoundary><Canvas key={canvasKey} frameloop="demand" camera={{position:[2.1,1.15,2.2],fov:42,near:.01,far:200}} dpr={[1,1.5]} raycaster={{params:{Points:{threshold:.008},Line:{threshold:.005},Mesh:{},LOD:{},Sprite:{}}}} gl={{antialias:true,powerPreference:"high-performance"}} onCreated={({gl})=>{gl.domElement.addEventListener("webglcontextlost",e=>{e.preventDefault();setContextLost(true);});gl.domElement.addEventListener("webglcontextrestored",()=>setContextLost(false));}}><color attach="background" args={["#04090f"]} /><Stars radius={75} depth={20} count={700} factor={.55} saturation={0} fade speed={0} /><Suspense fallback={null}>{env.data?<Earth3DSurface sun={env.data.sun_direction}/>:null}{geographyVisible?<GeographyLayer level={level}/>:null}</Suspense>{live?<CatalogPoints positions={positions} onSelect={onSelect} onHover={setHover} highlights={highlights}/>:null}{orbitVisible&&orbit.length>1&&level>0?<Line points={orbit} color="#62aebc" lineWidth={.85} transparent opacity={.5} depthWrite={false}/>:null}<SelectedMarker state={current}/><CameraRig command={command} state={current} onLevel={setLevel} sun={env.data?.sun_direction}/></Canvas></GlobeBoundary>
